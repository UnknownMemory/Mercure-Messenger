...
const Home = () => {
    ...
    const users = usersList.map(u => {
        return(<div key={u.id} onClick={() => handleClick(u.id)}>{u.username}</div>)
    })



    const handleMessage = (e) => {
        document.querySelector('#msg').insertAdjacentHTML('afterend', '<div class="alert alert-success w-75 mx-auto">Ping !</div>');
        window.setTimeout(() => {
